...
#  config.vbguest.installer_options = { allow_kernel_upgrade: true }
  ...
  config.vm.define "k8s-master" do |master|
    ...
    master.vm.network "private_network", ip: "192.168.55.30"
  ...
  (1..NodeCnt).each do |i|
    config.vm.define "k8s-node#{i}" do |node| 
      ...
      node.vm.network "private_network", ip: "192.168.55.#{i + 30}"
    end
  end
...
# permissive 모드로 SELinux 설정_현재 비활성화시킴
setenforce 0
...
# Hosts 등록
cat << EOF >> /etc/hosts
192.168.55.30 k8s-master
192.168.55.31 k8s-node1
192.168.55.32 k8s-node2
EOF
...
# 쿠버네티스 초기화 명령 실행
kubeadm init --apiserver-advertise-address 192.168.55.30 --pod-network-cidr=20.95.0.0/12
...
nohup kubectl proxy --port=4040 --address=192.168.55.30 --accept-hosts='^*$' >/dev/null 2>&1 &
